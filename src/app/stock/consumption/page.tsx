...
export default function GeneralConsumptionPage() {
  ...
  // --- Data Fetching Effect (Reverted to simpler loadedCount) ---
  useEffect(() => {
      ...
      onSnapshot(source.query,
        ...
        (error: FirestoreError) => {
          ...
          toast({
            ...
            description: `Permissão negada ou erro de rede. Detalhes: ${error.message}`,
            variant: "destructive",
            duration: 10000,
          });
          loadedCount++; 
  ...
  useEffect(() => {
    ...
    if ((role === 'hospital_operator' || role === 'ubs_operator') && associatedHospitalId) {
        ...
        if (associatedUnitId) {
            ...
            // For general operators, unitId should be selectable, so don't pre-fill it beyond resetting.
            locationForm.setValue('unitId', undefined, { shouldValidate: true }); 
        }
    } else if (role === 'admin' || role === 'central_operator') {
        // For admin/central_op, if a hospitalId was stuck from a previous non-admin render, clear it.
        // They should be able to select freely.
  ...
  }, [currentUserProfile, isLoadingData, locationForm, hospitals]); // hospitals added for safe reset
  ...
  useEffect(() => {
    if (currentUserProfile && (currentUserProfile.role === 'admin' || currentUserProfile.role === 'central_operator')) {
        // If hospitalId changes, and it's not the initial load/prefill, reset unitId
        // This allows admin/central_op to freely change hospital then unit
        // Check formState to avoid reset on initial mount if values are already set
  ...
  const handleLocationSubmit = (data: LocationSelectionFormData) => {
    ...
    let unitNameDisplay = hospital.name;
    ...
    if (data.hospitalId !== CENTRAL_WAREHOUSE_ID) {
      if (data.unitId === GENERAL_STOCK_UNIT_ID_PLACEHOLDER) {
        ...
        actualUnitIdForTransaction = undefined; // Ensure unitId is undefined for central
    }
    ...
    try {
      await runTransaction(firestore, (transaction) =>
        processMovementRowTransaction(
          ...
          // Pass the full UserProfile from AuthContext which should contain the UID as `id` if User extends UserProfile correctly
  ...
  if (isLoadingData && !currentUserProfile) { 
    return (
      <div className="flex h-screen items-center justify-center">
        {/* Reverted to simple text for hydration diagnosis based on previous error */}
        <span className="mr-2 text-primary">[Ícone Carregando...]</span>
        ...
        <Card className="shadow-lg">
          <Form {...locationForm}>
            <form onSubmit={locationForm.handleSubmit(handleLocationSubmit)}>
                          ...
                          <SelectContent>
                            ...
                            {availableUnitsForSelection
                             ...
                             {availableUnitsForSelection.length === 0 && isSelectedHospitalUBS && (!field.value || field.value !== GENERAL_STOCK_UNIT_ID_PLACEHOLDER) && 
                                <SelectItem value={GENERAL_STOCK_UNIT_ID_PLACEHOLDER} disabled={field.value === GENERAL_STOCK_UNIT_ID_PLACEHOLDER}>
                                    Selecione "Estoque Geral da UBS"
                                </SelectItem> 
                             }
                             {availableUnitsForSelection.length === 0 && !isSelectedHospitalUBS && <SelectItem value="" disabled>Nenhuma unidade cadastrada</SelectItem>}
                          </SelectContent>
                        </Select>
              ...
              <CardFooter>
                <Button type="submit" disabled={isLocationSubmitButtonDisabled || isLoadingData}>
